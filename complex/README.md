...
**Key Semantics**:
- Requires `hooks: [post-delete]` to be set explicitly — it is the only valid value, and the chart's schema rejects a `post-delete-job` component that omits it, so a copy-pasted `pre-job` component cannot accidentally run on install/upgrade instead of on uninstall.
